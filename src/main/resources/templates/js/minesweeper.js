...
function load() {
    minesweeper.Cell = function (row, col, content) {
        ...
        this.neighbors = function (board) {
            return this.vicinity().filter(function (loc) {
                return board.legal(loc.row, loc.col);
            }).map(function (loc) {
                return board.cellAt(loc.row, loc.col)
            });
        };

        this.maskedNeighbors = function (board) {
            return this.neighbors(board).filter(function (neighbor) {
                return !neighbor.revealed();
            });
        };
...
(function () {
    document.addEventListener('DOMContentLoaded', function () {
        ...
        load_(minesweeper.Grid(8, 8)(minesweeper.Generator(10/64)));
    });
}());
